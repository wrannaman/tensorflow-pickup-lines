...
summary_writer = tf.summary.FileWriter("twitterTuple_log/" + timestamp + "-training")

# Init for saving models. They will be saved into a directory named 'checkpoints'.
# Only the last checkpoint is kept.
if not os.path.exists("twitterTuple"):
    os.mkdir("twitterTuple")
...
for x, y_, epoch in txt.rnn_minibatch_sequencer(codetext, BATCHSIZE, SEQLEN, nb_epochs=20):
    ...
    # save a checkpoint (every 50 batches)
    #if step % 50 == 0:
        saver.save(sess, 'twitterTuple/rnn_train_' + timestamp, global_step=step)
